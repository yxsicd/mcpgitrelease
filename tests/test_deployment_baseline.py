...
class DeploymentBaselineTests(unittest.TestCase):
    ...
    def test_deployment_release_contains_read_only_preflight(self) -> None:
        ...
        self.assertIn("mcpgit-toolchain-init.sh", fetch)
        self.assertIn('chmod 0755 "$target/deploy/mcpgit-deploy.sh"', fetch)
        self.assertIn('"$target/deploy/mcpgit-toolchain-init.sh"', fetch)
