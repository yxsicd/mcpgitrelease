...
class ProductInstallerTests(unittest.TestCase):
    ...
    def test_product_entrypoints_are_executable_and_keep_the_fail_closed_contract(self) -> None:
        ...
        self.assertIn('"$install_tool" select --pointer', novice)
        helper = (ROOT / "scripts/install_state.py").read_text()
        self.assertIn("pointer needs both native architectures", helper)
    ...
    def test_online_bundle_helpers_follow_the_pinned_install_snapshot(self) -> None:
        ...
        self.assertIn('$MCPGIT_INSTALL_CONTENT_BASE/scripts/$helper', novice)
