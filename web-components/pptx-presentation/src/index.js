...
:host([data-css-fullscreen]) .card{border:0;border-radius:0;height:100vh}.bar{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 8px;background:#fafafa;border-bottom:1px solid #ddd}
.meta{font-size:11px;color:#666;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:60vw}.nav{display:flex;align-items:center;gap:4px}.nav button{display:inline-grid;place-items:center;width:28px;height:28px;border:1px solid #c7c7c7;background:#fff;padding:0;border-radius:7px;cursor:pointer;font-size:15px;line-height:1}.nav button:hover{background:#f1f2f3}.nav button:disabled{opacity:.35}
.page{min-width:44px;text-align:center;font-size:11px;font-weight:700}.shell{min-height:0;background:#d7d9dc;display:flex;align-items:center;justify-content:center;padding:12px;overflow:hidden}
...
class PptxPresentation extends HTMLElement{
  ...
  constructor(){super();this.attachShadow({mode:'open'}).append(template.content.cloneNode(true));this.$=s=>this.shadowRoot.querySelector(s);this.context=undefined;this._loadSeq=0;this._viewer=null;this._current=0;this._total=0;this._wheel=0;this._wheelLock=0;this._bytes=null;this._onKey=e=>this.#onKey(e)}
  ...
  download(filename){if(!this._bytes)return;const fallback=(this.src.split('/').pop()||'presentation').replace(/\.(m?js|ts)$/i,'')||'presentation';const name=filename||`${fallback}.pptx`;const blob=new Blob([this._bytes],{type:'application/vnd.openxmlformats-officedocument.presentationml.presentation'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;a.style.display='none';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),0);this.dispatchEvent(new CustomEvent('download',{detail:{src:this.src,filename:name}}))}
  async refresh(){if(this.src)return this.load(this.src)}
  async load(src){
    const seq=++this._loadSeq,started=performance.now();this._viewer=null;this._current=0;this._total=0;this._bytes=null;this.$('.viewer').replaceChildren();this.$('.loading').hidden=false;this.$('.loading').textContent='Loading presentation…';this.#sync();
    try{
      ...
      const result=normalizeBuildResult(await mod.buildDeck(PptxGenJS,this.context));const buildMs=performance.now()-buildStart;if(seq!==this._loadSeq)return;this._bytes=result.bytes;
